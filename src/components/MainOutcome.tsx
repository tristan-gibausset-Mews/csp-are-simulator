...
function badgeClasses(winner: WinningScenario): string {
  if (winner === 'CSP') return 'bg-csp/15 text-csp border border-csp/30';
  if (winner === 'ARE + préavis') return 'bg-are/15 text-are border border-are/30';
  return 'bg-mews-grey-100 text-mews-grey-500 border border-mews-grey-300/40';
}
...
export default function MainOutcome({ result }: MainOutcomeProps) {
  ...
  return (
    <section className="card relative overflow-hidden" aria-labelledby="main-outcome-title">
      <div aria-hidden="true" className="pointer-events-none absolute -right-20 -top-20 h-64 w-64 rounded-full bg-mews-accent/15 blur-3xl" />
      ...
      <div className="relative">
        ...
        <div className="mt-5 flex flex-col gap-4 border-t border-mews-grey-300/30 pt-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
